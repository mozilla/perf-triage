...
def parse_args():
    ...
    parser.add_argument(
        "email", help="email addresses of triage participants", nargs="+"
    )
...
def validate_args(args):
    ...
    if not date_pattern.match(args.date):
        print(
            'ERROR: provided date does not match expected format. Expected: "yyyy-mm-dd"',
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if num_emails != 3:
        print(
            "WARNING: expected 3 emails for triage: got {}.".format(num_emails),
            file=sys.stderr,
        )
